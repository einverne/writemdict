...
def _mdx_compress(data, compression_type=2):
    header = (struct.pack(b"<L", compression_type) +
              struct.pack(b">L", zlib.adler32(
                  data) & 0xffffffff))  # depending on python version, zlib.adler32 may return a signed number.
...
class MDictWriter(object):
    ...
    def __init__(self, d, title, description,
        ...
        d is a dictionary. The keys should be (unicode) strings. If used for a mdx
        ...
        is_mdd is a boolean specifying whether the file written will be a mdx file
